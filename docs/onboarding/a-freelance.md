# A. フリーランス / 副業エンジニア — CLI オンボーディングフロー設計

> **ステータス: 設計中（未実装）**
>
> このドキュメントはフリーランス / 個人事業主 / 副業エンジニア（ペルソナ A）向けの
> CLI オンボーディング体験の設計仕様です。
...
クライアント企業の private repo や自社 repo で作業しているフリーランス / 副業エンジニア。
月末の稼働集計・請求書作成が面倒で、自動化したい。
副業の場合は本業の合間に作業するため、稼働記録を忘れがち → 自動化ニーズが特に強い。
...
- サーバーから所属組織一覧を取得
- 1つしかなければ自動選択（表示のみ）
- サインアップ時に組織が自動作成されるため、通常は1つ以上存在する
- 「新しい組織を作成」は複数組織で運用したいケース向け
...
- 2回目以降も毎回サマリーを表示する（累計値）。初回と同じ「wow」を維持
- 新規アクティビティが0件でもサマリーは表示する
- 複数リポジトリが設定済みなら全部同期し、リポジトリごとに結果を表示
...
### 既に Web で使い始めてる人（ペルソナ B との合流）

管理者（ペルソナ B）がフリーランス兼任で CLI も使いたい場合：

1. Web のセットアップ（3 ステップ）は完了済み
2. ダッシュボード TODO の「CLI をセットアップする」リンクから案内
3. 作業リポジトリで `npx invoiceai` を実行
4. ログイン → 既存の Web アカウントで認証される
5. 組織が 1 つなら自動選択（Web で作成済みの組織）
6. クライアントも既存から選択（Web で登録済み）
7. 初回同期 → 完了

つまり CLI 側はフロー全体像と同じだが、Phase 3（組織選択）と Phase 4（クライアント選択）で既存データが選択肢に出るため、新規入力なしで進む。Web と CLI で別々にセットアップする必要はない。

### セッション期限切れ時の挙動

API が `401 Unauthorized` を返した場合、CLI は自動で再ログインフローを起動する：

```
ℹ セッションが期限切れです。再ログインします...
ブラウザでログインしてください...
...
> **GitHub App との併用**: 自社 Org の private repo で作業している場合は、GitHub App を自社 Org にインストールすることでサーバーサイドで自動収集も可能。CLI（クライアント repo 向け）と GitHub App（自社 repo 向け）は併用できる。どちらで収集されたアクティビティも同じ `activity` テーブルに統合される。
