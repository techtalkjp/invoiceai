...
export type TimesheetStoreApi = StoreApi<TimesheetState>

export function createTimesheetStore(): TimesheetStoreApi {
  return createStore<TimesheetState>((set, get) => ({
    // 選択状態
    selectedDates: [],
    isDragging: false,
    dragStartDate: null,
...
/**
 * タイムシートの store スコープを提供する Provider。
 * key prop と組み合わせることで月/クライアント切替時にストアごと破棄・再生成される。
 */
export function TimesheetProvider({ children }: { children: React.ReactNode }) {
  const storeRef = useRef<TimesheetStoreApi | undefined>(undefined)
  if (!storeRef.current) {
    storeRef.current = createTimesheetStore()
  }
  return (
    <TimesheetStoreContext value={storeRef.current}>
      {children}
...
/**
 * store API を取得（.getState() / .subscribe() 用）。
 * イベントハンドラやオートセーブなど、リアクティブでないアクセスに使う。
 */
export function useTimesheetStoreApi(): TimesheetStoreApi {
  const store = useContext(TimesheetStoreContext)
  if (!store) throw new Error('TimesheetProvider not found')
  return store
}

/**
 * セレクタ付き store フック。旧 useTimesheetStore(selector) と同じ使い方。
 * セレクタの戻り値が変わった時だけ再レンダリングされる。
 */
export function useTimesheetSelector<T>(
  selector: (state: TimesheetState) => T,
): T {
...
export function useIsSelected(date: string) {
  return useTimesheetSelector((state) => state.selectedDates.includes(date))
}

// 各行がハイライト状��のみ subscribe するセ��クタ
export function useIsHighlighted(date: string) {
  return useTimesheetSelector((state) => state.highlightedDates.includes(date))
}

// フィルタリング用: データがある日付のセットを subscribe するセレクタ
// monthData 全体ではなく「どの日にデータがあるか」の変化だけを検知する
export function useFilledDatesKey() {
  return useTimesheetSelector((state) => {
    const parts: string[] = []
...
export function useActivitiesForDate(date: string) {
  return useTimesheetSelector((state) => state.activitiesByDate[date])
}

// ���セルが自分のフィールドのみ subscribe す���セレクタ
export function useEntryField<K extends keyof TimesheetEntry>(
  date: string,
  field: K,
) {
  return useTimesheetSelector((state) => state.monthData[date]?.[field])
}
